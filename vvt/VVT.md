# Verzeichnis von Verarbeitungstätigkeiten (VVT)
### nach Art. 30 Abs. 1 DSGVO · URBANMAKER UG (haftungsbeschränkt) / leanAM

**Stand:** 18.06.2026

> Interne Dokumentation – nicht zur Veröffentlichung. Diese Datei direkt bearbeiten
> (kein Skript nötig). Bei einer neuen Verarbeitung einen neuen Abschnitt „VT-0X"
> ergänzen und oben das Datum aktualisieren.

---

## Verantwortlicher

- **Firma:** URBANMAKER UG (haftungsbeschränkt), Marke „leanAM"
- **Anschrift:** Geringhoffstraße 48, 48163 Münster, Deutschland
- **Geschäftsführer:** Juri Boos
- **Kontakt:** hq@leanam.com · +49 (0) 250 222 941 80
- **Datenschutzbeauftragter:** nicht bestellt (keine Pflicht nach Art. 37 DSGVO)

---

## VT-01 · Kontakt- und Demo-Anfragen über das Website-Formular
- **Status:** aktiv (seit Umstellung von Web3Forms auf Formcarry, 16.06.2026)
- **Zweck:** Beantwortung von Anfragen, Geschäftsanbahnung, Terminvereinbarung
- **Rechtsgrundlage:** Art. 6 Abs. 1 lit. b und lit. f DSGVO
- **Betroffene Personen:** Interessenten, (potenzielle) Kunden
- **Datenkategorien:** Name, Unternehmen, E-Mail, Telefon (optional), Funktion (optional), Thema, Nachricht
- **Empfänger / Auftragsverarbeiter:** Formcarry – Speicherung in EU-Rechenzentrum (Frankfurt am Main). AVV nach Art. 28 DSGVO (DPA: https://formcarry.com/legal/data-processing-agreement, PDF abgelegt am: 18.06.2026)
- **Drittlandtransfer:** nein (EU)
- **Löschfrist:** nach Bearbeitung der Anfrage, vorbehaltlich gesetzlicher Aufbewahrungsfristen
- **Schutzmaßnahmen (TOM):** TLS/HTTPS, Honeypot-Spamschutz, kein externes Captcha, Zugriff nur berechtigte Personen

## VT-02 · Terminbuchung (Demo / Erstgespräch) über Calendly
- **Status:** aktiv
- **Zweck:** Vereinbarung von Demo- und Beratungsterminen
- **Rechtsgrundlage:** Art. 6 Abs. 1 lit. b DSGVO
- **Betroffene Personen:** Interessenten, (potenzielle) Kunden
- **Datenkategorien:** Name, E-Mail, gewählter Termin, ggf. freiwillige Angaben
- **Empfänger / Auftragsverarbeiter:** Calendly LLC, Atlanta (USA). DPA: https://calendly.com/privacy
- **Drittlandtransfer:** ja (USA) – EU-US Data Privacy Framework + Standardvertragsklauseln (Art. 46 DSGVO)
- **Löschfrist:** nach Durchführung/Absage des Termins
- **Schutzmaßnahmen (TOM):** TLS/HTTPS; Calendly nur verlinkt, nicht eingebettet (Datenfluss erst nach Klick)

## VT-03 · Bereitstellung der Website und Server-Logs (Hosting)
- **Status:** aktiv
- **Zweck:** technische Auslieferung der Website, Stabilität und Sicherheit
- **Rechtsgrundlage:** Art. 6 Abs. 1 lit. f DSGVO
- **Betroffene Personen:** Website-Besucher
- **Datenkategorien:** IP-Adresse, Datum/Uhrzeit, Browser/User-Agent, Betriebssystem, abgerufene Ressourcen
- **Empfänger / Auftragsverarbeiter:** Cloudflare, Inc., San Francisco (USA). DPA vorhanden
- **Drittlandtransfer:** ja (USA) – EU-US Data Privacy Framework + Standardvertragsklauseln (Art. 46 DSGVO)
- **Löschfrist:** automatische Löschung/Anonymisierung der Logs nach kurzer Zeit (Cloudflare-Voreinstellung)
- **Schutzmaßnahmen (TOM):** TLS/HTTPS, selbst gehostete Schriften (keine externen CDNs), keine Tracking-Cookies

## VT-04 · E-Mail-Kommunikation
- **Status:** aktiv
- **Zweck:** Korrespondenz mit Interessenten, Kunden, Partnern
- **Rechtsgrundlage:** Art. 6 Abs. 1 lit. b und lit. f DSGVO
- **Betroffene Personen:** Kommunikationspartner
- **Datenkategorien:** Name, E-Mail, Inhalt, Metadaten
- **Empfänger / Auftragsverarbeiter:** AIXIV – Raoul König e.K., Engelbertstraße 16, 52078 Aachen, Deutschland (E-Mail-/Serverhosting; Server-Infrastruktur bei Hetzner Online GmbH, Deutschland). AVV nach Art. 28 DSGVO: bei AIXIV anzufragen
- **Drittlandtransfer:** nein (Deutschland/EU)
- **Löschfrist:** nach Wegfall des Zwecks und Ablauf gesetzlicher Aufbewahrungsfristen
- **Schutzmaßnahmen (TOM):** TLS, Zugriff nur berechtigte Personen

---

## Abgelöste Verarbeiter (Archiv)
- **Web3Forms (Spacedev, Indien):** war Formular-Backend für VT-01; abgelöst durch Formcarry (EU). Drittlandtransfer nach Indien entfällt. Eintrag zu Dokumentationszwecken behalten.
